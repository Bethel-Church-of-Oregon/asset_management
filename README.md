...
```
26-11001
│  │││└┴┴─ 물품 고유번호 3자리 (001~999, 등록 시 자동 부여)
│  ││└──── 관리 사역원 번호 1자리 (1 예배부, 2 선교부 …)
│  │└───── 건물/위치 번호 1자리 (1 비전, 2 은혜, 3 조이채플, 4 창고 …)
└──┴────── 취득 연도 2자리 (2026년 → 26)
```

건물/사역원 번호는 **설정** 화면에서 자유롭게 추가·수정할 수 있습니다.
고유번호는 `연도+건물+사역원` 조합별로 다음 번호가 자동 제안되며 직접 바꿀 수도 있습니다.
...
| 자산명 | 필수 |
| 관리 사역원 / 관리 팀명 | 사역원은 번호 체계, 팀명은 자유 입력 |
| 설치 · 보관 장소 | |
...
| `/settings` | 건물·사역원 코드 관리, 사용자 계정 관리 (관리자 전용) |
...
npm run db:seed        # 기준정보(건물·사역원) + 관리자 계정
```
...
**설정** 화면에서 실제 건물·사역원 코드와 사용자 계정을 정리하세요.
...
| **`users.username` 칸이 없음** | `npm run db:add-username` (아래 참고) |
