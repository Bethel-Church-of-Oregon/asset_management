 ...
 * 표기용이라 QR 에는 들어가지 않고(`assetNoBarcodeValue`), 입력·스캔은
...
/**
 * QR 에 인코딩할 값 — 하이픈을 뺀 10자리 숫자.
 *
 * 숫자만 남기면 QR 이 숫자 모드로 인코딩해 같은 내용이 더 작은 심볼에 들어갑니다
 * (`src/lib/qr.ts`). 덕분에 자산번호는 언제나 가장 작은 버전 1(21 × 21)에
 * 오류정정 H 를 걸고도 담기고, 같은 라벨에서 모듈이 그만큼 굵어집니다.
 * 사람이 읽는 줄에는 하이픈이 들어간 번호를 그대로 보여 주므로 화면 표기는
 * 달라지지 않습니다. 읽어 낸 `2601030001` 은 `parseAssetNo` 가 다시 해석합니다.
 */
