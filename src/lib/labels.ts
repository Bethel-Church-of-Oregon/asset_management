 ...
 * 출력하는 방식, `sheet` 는 Letter 라벨지에 여러 칸을 격자로 출력하는 방식입니다.
 *
...
export const LABEL_PRESETS: LabelPreset[] = [
  ...
  // ── Letter 라벨지 ────────────────────────────────────────────────────────
  // 교회는 오레곤에 있으므로 A4 가 아니라 Letter(8.5" × 11") 다. 아래 치수는
  // Avery 규격 그대로이고, 좌우·상하가 Letter 에 정확히 맞아떨어지는 것을
  // 확인했다 (여백 2 + 칸 × 개수 + 간격 = 215.90 / 279.40mm).
  {
    id: 'sheet-letter-30',
    name: 'Letter 라벨지 30칸 — Avery 5160 (3 × 10)',
    note: '주소 라벨 1" × 2-5/8". 가장 흔한 규격이라 어느 사무용품점에서나 구할 수 있습니다.',
    kind: 'sheet',
    widthMm: 66.68,
    heightMm: 25.4,
    paddingMm: 2,
    ...
    pageMarginLeftMm: 4.76,
    gapXMm: 3.18,
    gapYMm: 0,
  },
  {
    id: 'sheet-letter-14',
    name: 'Letter 라벨지 14칸 — Avery 5162 (2 × 7)',
    note: '4" × 1-1/3". 장소·팀명까지 넣어도 넉넉합니다.',
    kind: 'sheet',
    widthMm: 101.6,
    heightMm: 33.87,
    paddingMm: 2,
    ...
    cols: 2,
    rows: 7,
    pageMarginTopMm: 21.17,
    pageMarginLeftMm: 3.97,
    gapXMm: 4.76,
    gapYMm: 0,
  },
  {
    id: 'sheet-letter-10',
    name: 'Letter 라벨지 10칸 — Avery 5163 (2 × 5)',
    note: '4" × 2". 큰 장비용. QR 이 커서 멀리서도 읽힙니다.',
    kind: 'sheet',
    widthMm: 101.6,
    heightMm: 50.8,
    paddingMm: 3,
 ...
 * a Letter page with the label grid positioned by the paper's margins — 교회가
 * 오레곤에 있어 용지는 A4 가 아니라 Letter 다.
 *
...
${pagination}
  ...
  return `
@page { size: letter portrait; margin: ${layout.pageMarginTopMm}mm ${layout.pageMarginLeftMm}mm; }
.label-root { --label-w: ${layout.widthMm}mm; --label-h: ${layout.heightMm}mm; --label-p: ${layout.paddingMm}mm; }
.label-slot { display: contents; }
@media print {
  .label-root {
